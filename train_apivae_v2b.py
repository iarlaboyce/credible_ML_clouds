"""
Identifiability-fix variants of the v2 API-VAE training.

Mode 'pair'      -> models/apivae_weights_v2b.pth
  Physics L1 supervision on CLEAN pixels only, plus a pair-consistency
  loss: the polluted member of each pair must encode to the same zx as
  its clean twin (stop-gradient on the clean side). The model is never
  shown the counterfactual label for polluted pixels.

Mode 'cleanonly' -> models/apivae_weights_v2c.pth
  Physics L1 supervision on CLEAN pixels only, no pair term (March-style
  supervision on the v2 pipeline). Quantifies the identifiability failure.

Mode 'trueinit' -> models/apivae_weights_v2d.pth
  Same as 'cleanonly', but the plume module is initialised at the true
  generating parameters instead of the default init. Shows the clean-only
  objective contains a good solution -- it just isn't found from a generic
  starting point.

Mode 'oracle' -> models/apivae_weights_v2e.pth
  Physics L1 supervision on BOTH pair members using their true state,
  including the polluted member -- an upper bound unavailable in real
  observations, where the polluted member's clean state is exactly the
  unknown quantity. No pair-consistency term needed.

Usage: python train_apivae_v2b.py [pair|cleanonly|trueinit|oracle] [nc_filename] [suffix] [seed]
  nc_filename: default 'cloud_train_exp.nc' (in data/). PairDataset does its
               own dropna + inner-merge on pair_id, so the raw (non-NaN-
               filtered) remote_v3_runner.py output, e.g.
               'cloud_training_data_v3.nc', works directly here too.
  suffix:      default from MODE ('pair'->'v2b' etc). Override to avoid
               clobbering the existing weights/history for a given mode,
               e.g. pass 'v2b' explicitly when retraining v2b on v3 data
               (same suffix -> overwrites models/apivae_weights_v2b.pth).
  seed:        optional int; seeds torch/numpy before model construction so
               weight init (and hence, for cleanonly/pair, which basin the
               natural objective converges to) is reproducible. Omit to use
               whatever the ambient RNG state is (original behaviour).
"""
import sys
import torch
import torch.nn.functional as F
import torch.optim as optim
from torch.utils.data import DataLoader, Dataset, random_split
import xarray as xr
import numpy as np
import pandas as pd

sys.stdout.reconfigure(line_buffering=True)
sys.path.insert(0, 'src')
from model_exp import API_VAE
from neural_surrogate import SurrogateRTM

MODE = sys.argv[1] if len(sys.argv) > 1 else 'pair'
assert MODE in ('pair', 'cleanonly', 'trueinit', 'oracle')

EPOCHS        = 200
BATCH_PAIRS   = 256
LR            = 1e-3
WARMUP        = 10
MEAN_JACOBIAN = 21.13
LAMBDA_RECON  = 100.0
LAMBDA_PHYS   = 5.0
LAMBDA_PAIR   = 5.0 if MODE == 'pair' else 0.0   # cleanonly, trueinit & oracle: no pair term
LAMBDA_AUX    = 2.0
BETA_MAX      = 0.0005
GAMMA_MAX     = 0.001
NOISE_LEVEL   = 0.01
EARLY_STOP    = 20

NC_PATH      = f"data/{sys.argv[2]}" if len(sys.argv) > 2 else "data/cloud_train_exp.nc"
SCALERS_PATH = "data/surrogate_scalers_retrained.npz"
SUFFIX       = sys.argv[3] if len(sys.argv) > 3 else {'pair': 'v2b', 'cleanonly': 'v2c', 'trueinit': 'v2d', 'oracle': 'v2e'}[MODE]
SEED         = int(sys.argv[4]) if len(sys.argv) > 4 else None
WEIGHTS_OUT  = f"models/apivae_weights_{SUFFIX}.pth"
HISTORY_OUT  = f"results/apivae_{SUFFIX}_training_history.csv"


class PairDataset(Dataset):
    """One item per matched (clean, polluted) pair."""
    def __init__(self, nc_path, scalers_path):
        ds = xr.open_dataset(nc_path)
        df = ds.to_dataframe().reset_index().dropna(subset=["refl_2.1um", "refl_0.86um"])
        pol = df[df["type"] == "polluted"]
        cln = df[df["type"] == "clean"]
        p = pd.merge(pol, cln, on="pair_id", suffixes=("_pol", "_cln")).reset_index(drop=True)

        scalers = np.load(scalers_path)
        g_min = scalers["X_min"][3:].astype("float32")
        g_max = scalers["X_max"][3:].astype("float32")
        p_min = scalers["X_min"][:2].astype("float32")
        p_max = scalers["X_max"][:2].astype("float32")

        self.x_pol = p[["refl_2.1um_pol", "refl_0.86um_pol"]].values.astype("float32")
        self.x_cln = p[["refl_2.1um_cln", "refl_0.86um_cln"]].values.astype("float32")
        self.t_pol = np.column_stack((np.ones(len(p)),  p["age_pol"].values)).astype("float32")
        self.t_cln = np.column_stack((np.zeros(len(p)), p["age_cln"].values)).astype("float32")
        self.g_pol = ((p[["solz_pol", "satz_pol", "raz_pol"]].values.astype("float32") - g_min)
                      / (g_max - g_min))
        self.g_cln = ((p[["solz_cln", "satz_cln", "raz_cln"]].values.astype("float32") - g_min)
                      / (g_max - g_min))
        # clean-state target, normalised — this is the true counterfactual
        # for both pair members, since they share the same underlying clean
        # state; used against the clean member always, and additionally
        # against the polluted member in oracle mode
        tgt = p[["re_true_cln", "tau_true_cln"]].values.astype("float32")
        self.zx_tgt = (tgt - p_min) / (p_max - p_min)
        print(f"Pairs: {len(p)}", flush=True)

    def __len__(self):
        return len(self.x_pol)

    def __getitem__(self, i):
        return (torch.tensor(self.x_pol[i]), torch.tensor(self.x_cln[i]),
                torch.tensor(self.t_pol[i]), torch.tensor(self.t_cln[i]),
                torch.tensor(self.g_pol[i]), torch.tensor(self.g_cln[i]),
                torch.tensor(self.zx_tgt[i]))


def batch_trust(model, x, c_geom, c_track, veff_norm):
    """Per-sample trust weight from the surrogate Jacobian (inline, current encoder)."""
    with torch.set_grad_enabled(True):
        mu_x, _, _, _ = model.encoder(x, c_geom, c_track)
        zx = torch.sigmoid(mu_x).detach().requires_grad_(True)
        veff = torch.full_like(zx[:, 0:1], veff_norm)
        p_refl = model.surrogate(torch.cat([zx[:, 0:1], zx[:, 1:2], veff, c_geom], dim=1))
        jac = torch.autograd.grad(p_refl, zx, grad_outputs=torch.ones_like(p_refl))[0]
        j_norm = torch.norm(jac, dim=1, keepdim=True).detach()
    return 1.0 / (1.0 + (j_norm / MEAN_JACOBIAN).pow(2))


if __name__ == "__main__":
    device = torch.device("cuda")
    print(f"Mode: {MODE}  ->  {WEIGHTS_OUT}", flush=True)
    print(f"Training on: {torch.cuda.get_device_name(0)}", flush=True)
    if SEED is not None:
        torch.manual_seed(SEED)
        torch.cuda.manual_seed_all(SEED)
        np.random.seed(SEED)
        print(f"Seed: {SEED}", flush=True)

    scalers   = np.load(SCALERS_PATH)
    surrogate = SurrogateRTM().to(device)
    surrogate.load_state_dict(torch.load("models/surrogate_rtm_weights.pth",
                                         map_location=device, weights_only=True))
    surrogate.eval()

    model     = API_VAE(surrogate, scalers, device).to(device)

    if MODE == 'trueinit':
        # initialise the plume module AT the true generating parameters:
        # softplus(k_raw)=4.8, softplus(B_raw)=5.0, sigmoid(d_raw)=0.10
        import math
        with torch.no_grad():
            model.plume_network.k_eff.fill_(math.log(math.exp(4.8) - 1))
            model.plume_network.nd_boost.fill_(math.log(math.exp(5.0) - 1))
            model.plume_network.max_dilution.fill_(math.log(0.10 / 0.90))
        print(f"Plume init overridden: k={F.softplus(model.plume_network.k_eff).item():.3f} "
              f"B={F.softplus(model.plume_network.nd_boost).item():.3f} "
              f"d={torch.sigmoid(model.plume_network.max_dilution).item():.3f}", flush=True)

    optimizer = optim.Adam(model.parameters(), lr=LR)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, "min", factor=0.5, patience=5)

    dataset = PairDataset(NC_PATH, SCALERS_PATH)
    n       = len(dataset)
    train_n = int(0.85 * n)
    train_ds, val_ds = random_split(dataset, [train_n, n - train_n],
                                    generator=torch.Generator().manual_seed(42))
    train_loader = DataLoader(train_ds, batch_size=BATCH_PAIRS, shuffle=True, drop_last=True)
    val_loader   = DataLoader(val_ds,   batch_size=BATCH_PAIRS, shuffle=False)

    X_min_2d  = torch.tensor(scalers["X_min"][:2].astype("float32"), device=device)
    X_max_2d  = torch.tensor(scalers["X_max"][:2].astype("float32"), device=device)
    veff_norm = float((0.10 - scalers["X_min"][2]) / (scalers["X_max"][2] - scalers["X_min"][2]))

    best_val = float("inf")
    patience = 0
    history  = []

    for epoch in range(EPOCHS):
        model.train()
        anneal = min(1.0, (epoch + 1) / WARMUP)
        beta, gamma = BETA_MAX * anneal, GAMMA_MAX * anneal
        tot = {"recon": 0.0, "phys": 0.0, "pair": 0.0, "kl": 0.0}

        for xb_p, xb_c, tb_p, tb_c, gb_p, gb_c, tgt in train_loader:
            xb_p, xb_c = xb_p.to(device), xb_c.to(device)
            tb_p, tb_c = tb_p.to(device), tb_c.to(device)
            gb_p, gb_c = gb_p.to(device), gb_c.to(device)
            tgt = tgt.to(device)

            w_p = batch_trust(model, xb_p, gb_p, tb_p, veff_norm)
            w_c = batch_trust(model, xb_c, gb_c, tb_c, veff_norm)

            optimizer.zero_grad()
            out_p = model(xb_p + torch.randn_like(xb_p) * NOISE_LEVEL, tb_p, gb_p)
            out_c = model(xb_c + torch.randn_like(xb_c) * NOISE_LEVEL, tb_c, gb_c)
            (rec_p, mux_p, lvx_p, muy_p, lvy_p, zx_p, adv_p, aux_p) = out_p
            (rec_c, mux_c, lvx_c, muy_c, lvy_c, zx_c, adv_c, aux_c) = out_c

            recon_loss = (torch.mean(w_p * F.mse_loss(rec_p, xb_p, reduction="none"))
                          + torch.mean(w_c * F.mse_loss(rec_c, xb_c, reduction="none")))

            # physics supervision: CLEAN member only (oracle also supervises
            # the polluted member directly, below)
            phys_pred = zx_c * (X_max_2d - X_min_2d) + X_min_2d
            phys_true = tgt  * (X_max_2d - X_min_2d) + X_min_2d
            phys_loss = F.l1_loss(phys_pred, phys_true)
            if MODE == 'oracle':
                # oracle supervision gives the polluted member the true
                # counterfactual (clean-state) label directly -- the same
                # tgt used for the clean member, since both pair members
                # share the same underlying clean state
                phys_pred_pol = zx_p * (X_max_2d - X_min_2d) + X_min_2d
                phys_loss = phys_loss + F.l1_loss(phys_pred_pol, phys_true)

            # pair consistency: polluted zx chases clean-side zx (no label leak)
            pair_loss = F.l1_loss(torch.sigmoid(mux_p),
                                  torch.sigmoid(mux_c).detach())

            kl = sum(-0.5 * (1 + lv - mu.pow(2) - lv.exp()).sum()
                     for mu, lv in [(mux_p, lvx_p), (muy_p, lvy_p),
                                    (mux_c, lvx_c), (muy_c, lvy_c)]) / (2 * xb_p.size(0))

            adv_loss = (F.mse_loss(adv_p, zx_p.detach()) + F.mse_loss(adv_c, zx_c.detach()))
            aux_loss = (F.mse_loss(aux_p, gb_p) + F.mse_loss(aux_c, gb_c))

            loss = (LAMBDA_RECON * recon_loss + beta * kl + LAMBDA_PHYS * phys_loss
                    + LAMBDA_PAIR * pair_loss + LAMBDA_AUX * aux_loss + gamma * adv_loss)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 2.0)
            optimizer.step()

            tot["recon"] += recon_loss.item()
            tot["phys"]  += phys_loss.item()
            tot["pair"]  += pair_loss.item()
            tot["kl"]    += kl.item()

        model.eval()
        val_loss = 0.0
        with torch.no_grad():
            for xb_p, xb_c, tb_p, tb_c, gb_p, gb_c, _ in val_loader:
                xb_p, tb_p, gb_p = xb_p.to(device), tb_p.to(device), gb_p.to(device)
                xb_c, tb_c, gb_c = xb_c.to(device), tb_c.to(device), gb_c.to(device)
                rec_p, *_ = model(xb_p, tb_p, gb_p)
                rec_c, *_ = model(xb_c, tb_c, gb_c)
                val_loss += (F.mse_loss(rec_p, xb_p).item()
                             + F.mse_loss(rec_c, xb_c).item()) * len(xb_p)
        val_loss /= (n - train_n)
        scheduler.step(val_loss)

        nb = len(train_loader)
        k_eff    = F.softplus(model.plume_network.k_eff).item()
        nd_boost = F.softplus(model.plume_network.nd_boost).item()
        history.append({"epoch": epoch + 1, **{k: v / nb for k, v in tot.items()},
                        "val": val_loss, "k_eff": k_eff, "nd_boost": nd_boost})
        print(f"Ep {epoch+1:03d} | recon={tot['recon']/nb:.5f} | phys={tot['phys']/nb:.4f} | "
              f"pair={tot['pair']/nb:.4f} | val={val_loss:.6f} | k={k_eff:.3f} | "
              f"Nd={nd_boost:.2f}", flush=True)

        if val_loss < best_val:
            best_val = val_loss
            patience = 0
            torch.save(model.state_dict(), WEIGHTS_OUT)
        else:
            patience += 1
            if patience >= EARLY_STOP:
                print(f"Early stop at epoch {epoch + 1}", flush=True)
                break

    pd.DataFrame(history).to_csv(HISTORY_OUT, index=False)
    print(f"Done. Best val: {best_val:.6f}  ->  {WEIGHTS_OUT}", flush=True)
